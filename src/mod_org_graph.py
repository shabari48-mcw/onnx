...
model = onnx.load("D:/Learn DL/Emil-Net/src/simplified_emil_net.onnx")
...
new_where_output = gs.Variable(name=f"/modified_{original_output.name}", dtype=np.float32)
...
for node in graph.nodes:
    for i, inp in enumerate(node.inputs):
        if inp == original_output and node != where_node:
            ...
            print(f" Update Node Inputs Before Correction:{node.inputs}\n")
            node.inputs[i] = new_where_output
            print(f" Updated Node Inputs After Correction:{node.inputs}\n")


#Reshape node /planning_decoder/decoder_blocks.0/r2r_attn/Reshape_6 is taking the output of Where Node
